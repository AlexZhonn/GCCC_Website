...
import { Calendar, Clock, MapPin, Heart, Users, ExternalLink } from "lucide-react";
...
export default function CalendarEmbed({ currentLang }: CalendarEmbedProps) {
  ...
  const regularSchedules = [
    {
      title: { en: "Adult and Children Sunday School", zh: "主日學 (成人、青少年及兒童)" },
      time: { en: "Every Sunday at 9:30 AM", zh: "每週日上午 9:30 - 10:30" },
      room: { en: "Classrooms & Fellowship Halls", zh: "各小班教室及副堂" },
      desc: { en: "Interactive classes to study scriptures, theology, and discipleship suitable for all age brackets.", zh: "專為不同靈命階段設計的小組學堂，涵蓋聖經查考、神學初探與生活實踐。" },
      category: "discipleship"
    },
    {
      title: { en: "Bilingual Joint Sunday Worship", zh: "主日聯合崇拜" },
      time: { en: "Every Sunday at 10:50 AM", zh: "每週日上午 10:50 - 12:15" },
      room: { en: "Main Sanctuary & Online YouTube Live", zh: "教會大禮拜堂 及 官方YouTube直播" },
      desc: { en: "Our core corporate praise, biblical sermon delivery, Communion, and heartfelt fellowship.", zh: "甘城全體會眾合一頌揚基督、領受主言、聖餐擘餅並共享愛宴的重要崇拜。" },
      category: "worship"
    },
    {
      title: { en: "Friday Fellowship & Bible Studies", zh: "週五團契查經與學子愛宴" },
      time: { en: "Every Friday at 7:30 PM (Dinner at 6:30 PM)", zh: "每週五晚 7:30 (傍晚 6:30 愛宴)" },
      room: { en: "Entire Church Complex", zh: "教堂大樓各區小組教室" },
      desc: { en: "Specialized student food fellowships and cross-generational Bible study discussion rings.", zh: "最溫馨的週末補給站。備有家常菜餚，隨後依年齡和小組查經相交。" },
      category: "fellowship"
    },
    {
      ...
      desc: { en: "Intercessions for global missionaries, local communities, sickness, and spiritual growth.", zh: "為普世福傳、甘城當地復興、生病肢體及個人靈性成長代求守望。" },
      category: "prayer"
    }
  ];
  ...
  const t = {
    ...
    sub: { en: "Join Us Throughout the Week", zh: "在豐富的聚會日程中經歷主恩隨時相伴" },
    tabAgenda: { en: "Weekly Core Schedule", zh: "每週恆常聚會表" },
    tabIframe: { en: "Complete Google Calendar", zh: "完整 Google Calendar 互動日曆" },
    gcalDesc: { en: "This calendar tracks all upcoming church events, special retreats, and missions conferences.", zh: "日曆中收錄了教會近期所有特別培靈會、聯合宣教大會、特別洗禮及假期活動安排。" },
    openWindow: { en: "Open Google Calendar in New Tab", zh: "在新分頁中打開谷歌大日曆" }
  };

  // Google Calendar GCCC FL simulated public embed link (we frame it with responsive bounds)
  const gcalEmbedUrl = "https://calendar.google.com/calendar/embed?src=en.usa%23holiday%40group.v.calendar.google.com&ctz=America%2FNew_York"; // USA holidays as concrete default or real public map

  return (
    <div className="bg-[#FBFBF7] rounded-2xl border border-[#E7B7A0]/30 shadow-lg p-6 md:p-8" id="calendar_embed_section">
      ...
      {/* RENDER ACTIVE TAB */}
      {activeTab === "agenda" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="schedule-cards-grid">
              ...
              >
                ...
                <div className="border-t border-[#E7B7A0]/10 pt-4 mt-4 flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2 text-xs text-[#6F685B]">
                    ...
                    <span className="line-clamp-1">{sched.room[currentLang]}</span>
                  </div>
                </div>
              </div>
            );
